...
const messageRequestsJiraTask = (text: string) => {
    const normalized = text.toLowerCase()
    if (!normalized.includes("jira")) {
        return false
    }

    const mentionsArtifact = ["ticket", "task", "issue", "story"].some((term) =>
    ...
    const mentionsVerb = ["create", "open", "file", "raise", "log", "draft"].some((verb) =>
...
export default function Chat() {
    ...
    useEffect(() => {
        ...
        const lastUserPrompt = [...messages]
            ...
            .find(
                (message) =>
                    message.role === "user" && !isAutoGeneratedUserMessage(message),
            )
        ...
        const plainText = extractPlainText(lastUserPrompt)
        if (plainText.length === 0) {
            return
        }

        if (messageRequestsJiraTask(plainText)) {
