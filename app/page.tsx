...
const SESSION_MESSAGE_STORAGE_KEY = "strata.sessionHistory"
...
export default function Chat() {
    ...
    const sessionMessagesCacheRef = useRef<Record<string, UIMessage[]>>({})
    const pendingHistorySaveRef = useRef<number | null>(null)
    const restoredSessionIdRef = useRef<string | null>(null)
    const [isMessageCacheHydrated, setIsMessageCacheHydrated] = useState(false)
    ...
    useEffect(() => {
        if (typeof window === "undefined") {
            setIsMessageCacheHydrated(true)
            return
        }
        try {
            const stored = window.localStorage.getItem(SESSION_MESSAGE_STORAGE_KEY)
            if (stored) {
                const parsed = JSON.parse(stored) as Record<string, UIMessage[]>
                if (parsed && typeof parsed === "object") {
                    sessionMessagesCacheRef.current = parsed
                }
            }
        } catch (error) {
            console.error("Failed to load cached chat history", error)
        } finally {
            setIsMessageCacheHydrated(true)
        }
        return () => {
            if (pendingHistorySaveRef.current != null) {
    ...
    useEffect(() => {
        ...
        if (!sessionId || !isMessageCacheHydrated) {
            return
        }
        ...
        const hasManualMessages = messages.some(
            (message) => !isAutoGeneratedUserMessage(message),
        )
        if (hasManualMessages) {
            restoredSessionIdRef.current = sessionId
            return
        }
        const contextMessage = messages.find(isAutoGeneratedUserMessage)
        if (!contextMessage) {
            return
        }
        restoredSessionIdRef.current = sessionId
        setMessages([contextMessage, ...cachedMessages])
    }, [activeSession?.id, isMessageCacheHydrated, messages, setMessages])
    ...
    useEffect(() => {
        ...
        try {
            ...
            const serialized = JSON.stringify(manualMessages)
            const sanitized = JSON.parse(serialized) as UIMessage[]
            sessionMessagesCacheRef.current[sessionId] = sanitized
        } catch (error) {
        ...
        pendingHistorySaveRef.current = window.setTimeout(() => {
            ...
            try {
                window.localStorage.setItem(
                    ...
                    JSON.stringify(sessionMessagesCacheRef.current),
                )
            } catch (storageError) {
                console.error("Failed to persist chat history", storageError)
            }
        }, 500)
    }, [messages, activeSession?.id, isMessageCacheHydrated])
